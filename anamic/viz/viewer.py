...
class ImageViewer(param.Parameterized):
    ...
    color_mode_param = param.ObjectSelector(
        default="Single", objects=["Single", "Composite"])
    ...
    def __init__(self, image, dimension_order=None, enable_log=True, width=None,
        ...
        self.log = LoggingWidget(
            logger_name=f"Imageviewer-{self.viewer_id}", enable=enable_log)

        if image.dtype.name == 'float64':
            image = image.astype('float32')
            self.log.warning(
                "Image of type 'float64' have been converted to 'float32'")

        self.width = width
        self.height = height

        # Reshape the image
        self.image = reorder_image_dimensions(
            image, dimension_order=dimension_order)
        ...
        self.image_max = [self.image[:, i].max()
                          for i in range(self.image_channel)]
        self.image_min = [self.image[:, i].min()
                          for i in range(self.image_channel)]

        # Set intensities bounds.
        self.intensities_bounds = [
            (self.image_min[i], self.image_max[i]) for i in range(self.image_channel)]
        ...
        if self.image_channel > 1:
            ...
            self.channel_names = [
                f"Channel {i}" for i in range(self.image.shape[1])]
        ...
        if self.image.dtype.kind == 'f':
            bounds = np.finfo(self.image.dtype).min, np.finfo(
                self.image.dtype).max
        else:
            bounds = np.iinfo(self.image.dtype).min, np.iinfo(
                self.image.dtype).max
        ...
        self.param.colormap_param.objects = [
            name.capitalize() for name in self.palettes.keys()]
    ...
    def _plot_image(self):
        ...
        if self.color_mode_param == "Composite":
            ...
            if not self.color_mapper:
                ...
                self.color_mapper = bk.models.LinearColorMapper(
                    low=bounds[0], high=bounds[1], palette=palette)

            self.image_renderer = self.fig.image(
                color_mapper=self.color_mapper, **image_args)

            # Add colorbar
            self.color_bar = bk.models.ColorBar(
                color_mapper=self.color_mapper, location=(0, 0))
        ...
        values = ", ".join(
            [f'@channel_{i}' for i in range(self.image_channel)])
    ...
    def _update_image_view(self):
        ...
        if self.color_mode_param == "Composite":
            ...
            for i, bound in enumerate(self.intensities_bounds):
                images[i] = skimage.exposure.rescale_intensity(
                    images[i], in_range=bound)
            ...
            frame = skimage.exposure.rescale_intensity(
        ...
        for i in range(self.image_channel):
            metadata[f'channel_{i}'] = [
                self.image[self.time_param, i, self.z_param]]
    ...
    def _update_intensities_range(self):
        ...
        if self.color_mapper:
            channel_index = self._get_channel_index()
            self.intensities_bounds[channel_index] = tuple(
                self.intensities_param)
    ...
    def _update_colormap(self):
        ...
        """
        if self.color_mapper:
            self.color_mapper.palette = self.palettes[str(
                self.colormap_param).lower()]
            self._update_fig()
    ...
    def _get_image_info(self):
        ...
        if self.image_channel > 1:
            ...
            infos.append(('Channel', self.image_channel,
                          self.channel_names.index(channel_index)))
    ...
    def panel(self):
        ...
        info_container = pn.Column(
            pn.pane.Markdown('## Informations'), info_widget)
        ...
        parameters_container = pn.Column(pn.pane.Markdown(
            '## Viewer Configurations'), parameters_widget)
