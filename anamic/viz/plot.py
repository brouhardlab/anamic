...
def viz_dimers(dimers, start_row=0, grid=True):
    ...
    if grid:
        ax.set_xticks(np.arange(start_row - 0.5,
                                dimers.shape[1] - 0.5), minor=True)
...
def viz_2d_dimers_positions(ax, dimers, x_feature, y_feature, color_feature, marker_size, x_offset=None, colormap='tab20c'):
    ...
    ax.scatter(dimers[x_feature], dimers[y_feature],
               c=colors, s=marker_size, cmap=colormap)
    ax.set_aspect('equal')
    if x_offset:
        ax.set_xlim(dimers[x_feature].max() - x_offset,
                    dimers[x_feature].max() + 5)
    else:
...
def show_tips(ax, positions, coordinates_features, marker_size=80):
    x_start, x_end, y_start, y_end = structure.get_mt_tips(
        positions, coordinates_features)
    ax.scatter([x_start], [y_start], color='cyan',
               label='Start', marker='x', s=marker_size)
    ax.scatter([x_end], [y_end], color='red',
               label='End', marker='x', s=marker_size)
    ax.legend()
